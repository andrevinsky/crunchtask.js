...
(function (){
  ...
  var Promise = (typeof Promise !== 'undefined') ? Promise : false;
  ...
  if ((typeof require !== 'undefined') && (Promise === false)) {
  ...
  function isExecutable(vArg) {
	...
	CrunchTask.for = function(_args_, _task_/*start, finish, increment*/){
  ...
  CrunchTask.normalizeRanges = function(){
    var args = __slice.call(arguments, 0);
    return translateRangeArgs(parseRangeArgs(args));
  };

  CrunchTask.next = tryAdvance;
  ...
  function _internalFor(_ranges,  taskBody, taskTail){
    return new CrunchTask(function(init, body, fin){
      ...
      body(function(resolve){
        callExecutableWith(taskBody, getRangeConditions(ranges));
        if (!tryAdvance(ranges)) resolve();
      }, 100);
      fin(function(){
        callExecutableWith(taskTail, getRangeConditions(ranges));
      });
    });
  }
  ...
  function getRangeConditions(ranges){
    var result = [];
  ...
  function parseRangeArgs(args, _acceptOnlyNumbers) {
    var result = [],
        current = [],
        item,
        isArray;
    ...
    while (args.length) {
      ...
      if (isArray && (_acceptOnlyNumbers !== true)) {
        if (current.length) {
          result.push(current);
          current = [];
        }
    ...
    return (_acceptOnlyNumbers === true) ? current : result;
  }
