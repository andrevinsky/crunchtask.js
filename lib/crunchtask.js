...
(function (){
  ...
  var fnIsArray = Array.isArray || function(val) {
        return __toString.call(val) === '[object Array]';
      };

  function isFunction(val) {
    return __toString.call(val) === '[object Function]';
  }

  function isExecutable(val) {
    return isFunction(val) || (val  instanceof CrunchTask);
  }

  function isNumber(val) {
    return __toString.call(val) === '[object Number]';
  }
  ...
  function _fnPartial(/*{ctx}, fn, args..*/){
    ...
    if ((!isFunction(fn)) && (isFunction(args0[0]))) {
      ctx = arguments[0];
      fn = args0.shift();
    }
    return function(){
  ...
  function _fnWrapArgsFor(fn /*, auxFns...*/) {
    ...
    return function(){
      var args = __slice.call(arguments, 0),
          auxFn;
      while ((auxFn = auxFns.shift()) && isFunction(auxFn)) {
  ...
  function _trigger(hive, evt /*, args*/){
    ...
    if ((args1.length === 1) && (fnIsArray(args1[0]))) {
      // unwrap
      args1 = args1[0];
    }
    if (!hive[evt]) { return; }
  ...
  function proceedBody(bodyFn, needRepeat, timeoutAmount, finallyFn, controlNexus) {
    var resolve = controlNexus.resolve,
        ...
        timeLimit = isNumber(needRepeat) ? needRepeat : 0;
  ...
  function CrunchTask(descriptionFn){
    ...
    if (!isFunction(descriptionFn)) {
      throw new Error('Single argument is required: `fn(initFn,bodyFn,finFn)`');
    }
  ...
  function crunchTaskFor (/*{start, finish, increment,} * n, fnBody, {fnTail}*/){
    ...
    var args = __slice.call(arguments, 0, argsCount - fnCount),
        ...
        taskBody = fns[0], taskTail = fns[1] || function(){};
