...
(function (){
  ...
  if (Promise === false) {
    throw new Error('Dependency isn\'t met: "promise-polyfill"');
  }
  ...
  CrunchTask.for = crunchTaskFor;
  CrunchTask.normalizeRanges = normalizeRanges;
  CrunchTask.next = canAdvance;


  var fnIsArray = Array.isArray || function(value) {
        return __toString.call(value) === "[object Array]"
      };

  function isFunction(vArg) {
    return __toString.call(vArg) === "[object Function]";
  }

  function isExecutable(vArg) {
    return isFunction(vArg) || (vArg  instanceof CrunchTask);
  }
	...
	function _fnPartial(fn){
		var ctx = this, args0 = __slice.call(arguments, 1);
	...
	function _fnWrapArgsFor(fn /*, aux...*/) {
		var args0 = __slice.call(arguments, 1);
		return function(){
			var args1 = __slice.call(arguments, 0), auxFn;
  ...
  function crunchTaskFor (_args_, _task_/*start, finish, increment*/){
    var argsCount = arguments.length, fnCount = 0;
    ...
    var ranges = translateRangeArgs(parseRangeArgs(args));
    var taskBody = fns[0], taskTail = fns[1] || function(){};

    if (!taskBody) return _resolvedTask();
  ...
  function _internalFor(_ranges,  taskBody, taskTail){
    return new CrunchTask(function(init, body, fin){
      ...
      body(function(resolve){
        if (canRunCycle) {
          callExecutableWith(taskBody, collectCurrentRanges(ranges));
        }
        if (!canRunCycle || !canAdvance(ranges)) resolve();
      }, 100);
      fin(function(){
        callExecutableWith(taskTail, collectCurrentRanges(ranges));
      });
    });
  }
  ...
  function collectCurrentRanges(ranges){
    var result = [];
  ...
  function canAdvance(ranges, justCheck){
    var currentR = 0, maxR = ranges.length, range, counterOverflow, rangeInBounds;
    do {
  ...
  function parseRangeArgs(args, preventRecursion) {
    ...
    while (args.length) {
      ...
      isArray = fnIsArray(item);

      if (isArray && (preventRecursion !== true)) {
        if (current.length) {
          result.push(current);
          current = [];
        }
    ...
    return (preventRecursion === true) ? current : result;
  }

  function translateRangeArgs(args) {
    var result = [], current;
    for (var i = 0, maxI = args.length; i < maxI; i++) {
      current = translateRangeSet(args[i]);
      for(var j = 0, maxJ = current.length; j < maxJ; j++)
  ...
  function translateRangeSet(rangeSet) {
    ...
    while(rangeLength = rangeSet.length) {
      splitBase = (((rangeLength % 3) === 0) || ((rangeLength > 3) && (((rangeLength + 1) % 3) === 0))) ? 3 : 2;
      current = rangeSet.splice(0, splitBase);
      if (current.length > 1) {
        result.push(translateRangeItem(current));
      }
    }
    return result;
  }

  function translateRangeItem(rangeItem) {
    var result = {
      from: rangeItem[0] || 0,
      to: (typeof rangeItem[1] !== 'undefined') ? rangeItem[1] : rangeItem[0] || 0,
      current: rangeItem[0] || 0
    };

    result.step = rangeItem[2] || ((result.from < result.to) ? 1 : -1);
