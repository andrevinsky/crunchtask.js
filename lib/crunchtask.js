...
(function (){
	...
	CrunchTask.for = function(task/*start, finish, increment*/){
		var args = [].slice.call(arguments, 1);
		var internalTask = new CrunchTask(function(init, body, fin){
			var depth, mins = [], maxes = [], steps = [], currents = [];
			body(function(){
				if (typeof task === 'fucntion') {
					task();
				} else if (task instanceof CrunchTask){
					task.run.apply(task, currents);
				}
				// increments
				// check if resolve
